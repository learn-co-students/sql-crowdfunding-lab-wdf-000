...
"SELECT projects.title, SUM(pledges.amount) - projects.funding_goal AS over_amount FROM projects
JOIN pledges ON pledges.project_id = projects.id
GROUP BY pledges.project_id
...
"SELECT users.name, SUM(pledges.amount) AS amount_sum FROM users
JOIN pledges ON pledges.user_id = users.id
GROUP BY users.name
