...
"SELECT projects.title, (SUM(pledges.amount) - projects.funding_goal) AS over  FROM projects INNER JOIN pledges ON projects.id = pledges.project_id GROUP BY projects.title HAVING over >= 0;"
end
