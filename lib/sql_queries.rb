...
"SELECT users.name, users.age, SUM(pledges.amount) FROM users JOIN pledges ON user_id = users.id GROUP BY users.name"
end
...
def selects_user_names_and_amounts_of_all_pledges_grouped_by_name_then_orders_them_by_the_amount
 "SELECT users.name, SUM(pledges.amount) FROM pledges JOIN users ON users.id = user_id GROUP BY users.name ORDER BY SUM(pledges.amount);"
