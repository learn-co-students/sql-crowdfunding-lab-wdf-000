...
"SELECT projects.title, (SUM (pledges.amount) - projects.funding_goal) AS over_fgoal  FROM projects INNER JOIN pledges ON projects.id = pledges.project_id GROUP BY projects.title HAVING SUM (pledges.amount) >= projects.funding_goal;"
end
...
"SELECT projects.category, SUM(pledges.amount) FROM projects INNER JOIN
pledges on projects.id = pledges.project_id WHERE projects.category = 'books' GROUP BY projects.category;"
end
